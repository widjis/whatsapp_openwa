...
Copied phone numbers may include invisible direction marks (production example: U+202A/U+202C around entry 3). SRF phone validation now removes known directional/zero-width format controls before validating; visible invalid text still fails with its entry index. For existing containers, manually retype the affected .env line using plain ASCII and recreate with `restart --no-build`, or sync the updated parser and rebuild with `deploy`. A successful parse does not resend an already-delivered SRF automatically.
